...
class DeeperCNN(nn.Module):
    def __init__(self):
        ...
        self.fc_layer = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Dropout(p=0.5),
            nn.Linear(512, 4)  # Assuming 4 classes for emotion classification
        )
...
for epoch in range(num_epochs):
    ...
    print(f'Epoch [{epoch + 1}/{num_epochs}], Loss: {epoch_loss:.4f}, Learning Rate: {scheduler.get_last_lr()[0]:.6f}')
...
# Save the best model
torch.save(model.state_dict(), 'Models/model_variation2.pth')