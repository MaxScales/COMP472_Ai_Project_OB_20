...
if not os.path.isdir(data_path):
    raise FileNotFoundError(f"Check if the Data directory exists at {script_dir}")
...
files, labels, class_names = load_images_and_labels(data_path)

# Create dataset
dataset = CustomDataset(files, labels, transform=transform)
...
for epoch in range(num_epochs):
    ...
    print(f'Epoch [{epoch + 1}/{num_epochs}], Loss: {epoch_loss:.4f}, Accuracy: {epoch_acc:.4f}, Learning Rate: {scheduler.get_last_lr()[0]:.6f}')
